...
void
tasyclCreateQueues(size_t count=TaskingModel::getNumCPUs(), bool share_context, bool inOrderQueues)
{
    if (count == TASYCL_NQUEUES_AUTO)
        count = TaskingModel::getNumCPUs();
	assert(count > 0);

	QueuePool::initialize(count, share_context, inOrderQueues);
}
